...
function get_warehouse_id_attr_number($goods_id, $attr_id = '', $admin_id, $warehouse_id, $area_id, $model_attr = '', $store_id = 0)
{
	...
	$sql = 'SELECT product_number, product_sn ' . $bar_code . ' FROM ' . $GLOBALS['ecs']->table($table) . ' WHERE goods_id = \'' . $goods_id . '\'' . $where . ' ORDER BY product_id DESC LIMIT 1';
	return $GLOBALS['db']->getRow($sql);
}
...
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $region = array(), $seckill_price = '')
{
