	...
	public function set_cookie($username = '', $remember = NULL, $is_oath = 1)
	{
		if (empty($username)) {
			...
			if ($is_oath == 1) {
				...
				$sql = 'SELECT u.user_id, u.user_name, u.password, u.email, ua.identifier FROM ' . $GLOBALS['ecs']->table('users') . ' AS u,' . $GLOBALS['ecs']->table('users_auth') . ' AS ua' . ' WHERE u.user_id = ua.user_id AND ua.user_name = \'' . $username . '\' LIMIT 1';
				$row = $GLOBALS['db']->getRow($sql);
			}
	...
	public function set_session($username = '', $is_oath = 1)
	{
		if (empty($username)) {
			...
			if ($is_oath == 1) {
				...
				$sql = 'SELECT u.user_id, u.user_name, u.password, u.email FROM ' . $GLOBALS['ecs']->table('users') . ' AS u WHERE ' . $field_name . ' LIMIT 1';
				$row = $GLOBALS['db']->getRow($sql);
			}
			else {
				$sql = 'SELECT u.user_id, u.user_name, u.password, u.email, ua.identifier FROM ' . $GLOBALS['ecs']->table('users') . ' AS u,' . $GLOBALS['ecs']->table('users_auth') . ' AS ua' . ' WHERE u.user_id = ua.user_id AND ua.user_name = \'' . $username . '\' LIMIT 1';
