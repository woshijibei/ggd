...
function get_store_shop_goods_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id)
{
	...
	$select = 'g.goods_id, g.sales_volume, g.goods_thumb,g.is_shipping, g.goods_name, g.user_id, g.promote_start_date, g.promote_end_date, g.market_price, msi.self_run, ';
...
{
	$sql = 'SELECT cat_id, cat_name, keywords, cat_desc, cat_icon, pinyin_keyword, cat_alias_name, style_icon FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC limit 10';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	...
	foreach ($res as $key => $row) {
	    ...
	    $arr_temp=get_digui($row['cat_id']);
	    if(count($arr_temp)>0){
	    	$arr[$row['cat_id']]['arr'] = $arr_temp;
	    	$arr[$row['cat_id']]['count_arr'] = 1;
	    }
	    else{
	    	$arr[$row['cat_id']]['count_arr'] = 0;  
	    }
...
function get_digui($cat_id)
{
	$sql = 'SELECT cat_id, cat_name, keywords, cat_desc, cat_icon, pinyin_keyword, cat_alias_name, style_icon FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' WHERE parent_id = '.$cat_id.' AND is_show = 1 ORDER BY sort_order ASC ';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
    if(!empty($res)){
    	foreach ($res as $key => $row) {
			...
			$arr[$row['cat_id']]['style_icon'] = $row['style_icon'];
			$arr[$row['cat_id']]['url'] = build_uri('wholesale_cat', array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
		    $arr[$row['cat_id']]['arr'] = get_digui($row['cat_id']);
		}
...
function get_filter_goods_list($filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0), $size = 10, $page = 1, $sort = 'sort_order', $order = 'ASC', $warehouse_id = 0, $area_id = 0, $type = '')
{
	$where = ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ';
