...
function get_purchase_list($filter = array(), $size = 10, $page = 1, $sort = 'add_time', $order = 'DESC') 
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) 
	{
		...
		$row['left_day'] = (0 < $row['left_day'] ? $row['left_day'] : 0);
		$row['add_time'] = local_date('Y-m-d', $add_time);//修改时间 2017 09 23
		...
		$row['url'] = build_uri('purchase', array('pid' => $row['purchase_id'], 'act' => 'info'), $row['subject']);
		$arr[] = $row;
	}
...
function wholesale_list($ru_id) 
{
	...
	if ($result === false) 
	{
		...
		$filter = page_and_size($filter);//修改时间 2017 09 23
...
function set_wholesale_goods_attribute($goods_type = 0, $goods_id = 0, $goods_model = 0) 
{
	...
	$attribute_list = $GLOBALS['db']->getAll($sql);//修改时间 2017 09 23
...
function get_wholesale_goods_info($act_id, $warehouse_id = 0, $area_id = 0, $select = array())     
{
	...
	$left_join .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS bd ON bd.brand_id = g.brand_id ';  
	...
	$sql = ' SELECT bd.brand_logo,w.goods_id, w.goods_name, w.goods_type, w.rank_ids, w.price_model, ' . $goods_price . ', w.goods_number, w.moq, ' . ' wc.cat_id, ' . ' g.user_id, g.market_price, g.shop_price, g.goods_desc, g.goods_img, g.brand_id, g.goods_sn, g.goods_weight FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . $left_join . $where;
	$row = $GLOBALS['db']->getRow($sql);
	$arr = array();
	if ($row !== false) 
	{
		$arr['goods_id'] = $row['goods_id'];
		$arr['goods_type'] = $row['goods_type'];
		$arr['user_id'] = $row['user_id'];
		$arr['goods_name'] = $row['goods_name'];
		$arr['rank_ids'] = $row['rank_ids'];
		$arr['price_model'] = $row['price_model'];
		$arr['goods_price'] = $row['goods_price'];
		$arr['goods_price_formatted'] = price_format($row['goods_price']);
		$arr['goods_number'] = $row['goods_number'];
		$arr['moq'] = $row['moq'];
		$arr['volume_price'] = get_wholesale_volume_price($row['goods_id']);
		$arr['cat_id'] = $row['cat_id'];
		$arr['market_price'] = $row['market_price'];
		$arr['shop_price'] = $row['shop_price'];
		$arr['goods_desc'] = $row['goods_desc'];
		$arr['brand_id'] = $row['brand_id'];
		$arr['goods_extend'] = get_wholesale_extend($row['goods_id']);
		$arr['goods_img'] = get_image_path($row['goods_id'], $row['goods_img']);
		$arr['shopinfo'] = get_shop_name($row['user_id'], 2);
		$arr['shopinfo']['logo_thumb'] = str_replace(array('../'), '', $arr['shopinfo']['logo_thumb']);
		$arr['goods_sn'] = $row['goods_sn'];
		$arr['goods_weight'] = (0 < intval($row['goods_weight']) ? $row['goods_weight'] . $GLOBALS['_LANG']['kilogram'] : ($row['goods_weight'] * 1000) . $GLOBALS['_LANG']['gram']);
		$arr['rz_shopName'] = get_shop_name($row['user_id'], 1);
		$build_uri = array('urid' => $row['user_id'], 'append' => $arr['rz_shopName']);
		$domain_url = get_seller_domain_url($row['user_id'], $build_uri);
		$arr['store_url'] = $domain_url['domain_name'];

		if ($GLOBALS['_CFG']['open_oss'] == 1) 
		{
			$bucket_info = get_bucket_info(); 
			$arr['shopinfo']['brand_thumb'] = $bucket_info['endpoint'] . DATA_DIR . '/brandlogo/'. $row['brand_logo'];
		    if(!strpos($arr['goods_desc'],$bucket_info['endpoint'])){
                $arr['goods_desc']=str_replace('src="','src="'.$bucket_info['endpoint'],$row['goods_desc']);
		    }
		    
		}   
		else{
            $arr['shopinfo']['brand_thumb'] = '/'. DATA_DIR . '/brandlogo/'. $row['brand_logo'];  
		} 
...
function see_more_goods($user_id, $act_id) 
{
	...
	$sql = ' SELECT w.act_id, ' . $goods_price . ', g.goods_name, g.goods_thumb, g.user_id FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = w.goods_id ' . ' WHERE g.user_id = \'' . $user_id . '\' AND w.act_id <> \'' . $act_id . '\' ORDER BY act_id DESC LIMIT 5 ';
	$res = $GLOBALS['db']->getAll($sql);
	if ($res) 
	{
		$arr = array();
		foreach ($res as $k => $v ) 
		{
			...
			{
				$bucket_info = get_bucket_info();
				$arr[$k]['goods_thumb'] = $bucket_info['endpoint'] .$v['goods_thumb'];;
			}
			else{
	            $arr[$k]['goods_thumb'] = $v['goods_thumb'];;  
			} 
...
function wholesale_cart_goods($goods_id = 0, $rec_ids = '') 
{
	...
	foreach ($ru_ids as $key => $val ) 
	{
		...
		foreach ($goods_ids as $a => $g ) 
		{
            ...
            if($GLOBALS['_CFG']['open_oss'] == 1){
                 $goods_data['goods_thumb'] =$bucket_info['endpoint'] . $goods_data['goods_thumb'];
            }
...
function get_wholesale_orders($user_id, $record_count, $page, $where = '', $order = '', $pagesize = 10) 
{
	...
	$arr = array();
	$sql = 'SELECT oi.pay_id,oi.pay_status,og.ru_id,og.goods_attr ,og.goods_number, oi.inv_type, oi.inv_payee, oi.tax_id, oi.postscript, oi.main_order_id, oi.consignee, oi.order_id,oi.mobile, oi.order_sn, oi.order_status, oi.add_time, ' . '(oi.order_amount) AS total_fee, ' . 'og.goods_id, oi.email, oi.address, oi.province, oi.city, oi.district ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi' . ' left join ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og on oi.order_id = og.order_id' . $left_join . ' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = 0 ' . $where . ' and (select count(*) from ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi2 where oi2.main_order_id = oi.order_id) = 0 ' . 'group by oi.order_id ORDER BY oi.add_time DESC ' . $limit;
	$res = $GLOBALS['db']->query($sql);  
	while ($row = $GLOBALS['db']->fetchRow($res)) 
	{
...
function wholesale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($res === NULL) {
		...
		if ($data === false) {
			$sql = 'SELECT w.is_show,w.cat_id, w.cat_name,w.is_show, w.parent_id, w.sort_order, COUNT(s.parent_id) AS has_children ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS w ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS s ON s.parent_id = w.cat_id ' . 'GROUP BY w.cat_id ' . 'ORDER BY w.parent_id, w.sort_order ASC';
			$res = $GLOBALS['db']->getAll($sql);
