...
function order_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete,' . 'o.shipping_fee, o.insure_fee, o.pay_fee, o.surplus,o.tax, o.integral_money, o.bonus, o.discount, o.coupons,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code AS o_extension_code, ' . 'o.extension_id, o.user_id, o.referer, o.froms, o.chargeoff_status, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order, o.pay_id ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id ' . $leftJoin . $where . $where_store . $no_main_order . $groupBy . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
	...
	foreach ($row as $key => $value) {
		...
		$sql = 'SELECT ru_id, extension_code AS iog_extension_code FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE order_id = \'' . $value['order_id'] . '\' LIMIT 1';
		$order_goods = $GLOBALS['db']->getRow($sql);
		$value['ru_id'] = $order_goods['ru_id'];
		$row[$key]['iog_extension_code'] = $order_goods['iog_extension_code'];
...
function download_orderlist($result)
{
	...
	$data = i('订单号,' . $data_name . '下单会员,下单时间,收货人,联系电话,地址,总金额,配送费用,保价费用,支付费用,余额金额,积分金额,红包金额,发票税额,折扣金额,优惠券金额,应付金额,确认状态,付款状态,发货状态,订单来源,支付方式' . "\n");
	$count = count($result);
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$data .= $order_sn . ',' . $ru_name . $order_user . ',' . $order_time . ',' . $consignee . ',' . $tel . ',' . $address . ',' . i($result[$i]['total_fee']) . ',' . $shipping_fee . ',' . $insure_fee . ',' . $pay_fee . ',' . $surplus . ',' . $integral_money . ',' . $bonus . ',' . $tax . ',' . $discount . ',' . $coupons . ',' . $order_amount . ',' . $order_status . ',' . $pay_status . ',' . $shipping_status . ',' . $froms . ',' . $pay_name . "\n";
	}
...
if ($_REQUEST['act'] == 'order_query') {
	...
	if (file_exists($file) && (0 < $order['user_id'])) {
		...
		$pushData = array(
			...
			'keyword3' => array('value' => $order['invoice_no']),
	...
	if ($type == 1) {
		...
		if (0 < $ret_d) {
			$refound_fee = order_refound_fee($order_id, $ret_d);
		...
		$is_refound_shippfee = order_refound_shipping_fee($order_id, $ret_d);
