...
function copy_img($image = '', $type = 'goods')
{
	$newname = '';
	...
	if ($image) {
		...
		$newname = dirname($img) . '/' . $rand_name . $img_ext;

		if ($GLOBALS['_CFG']['open_oss'] == 1) {
			$bucket_info = get_bucket_info();
			$url = $bucket_info['endpoint'] . $image;
			get_http_basename($url, $newname);
		}
	...
	get_oss_add_file(array($newname));
...
if ($_REQUEST['act'] == 'list') {
	...
	if (!$db->getOne(' SELECT goods_id FROM ' . $ecs->table('goods') . ' WHERE goods_id =\'' . $goods['lib_goods_id'] . '\' AND user_id = \'' . $ru_id . '\' ')) {
		...
		if ($res) {
			foreach ($res as $k => $v) {
				$img_url = copy_img($v['img_url'], 'gallery');
				$thumb_url = copy_img($v['thumb_url'], 'gallery_thumb');
				$img_original = copy_img($v['img_original'], 'gallery');
