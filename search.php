	...
	if ($search_type == 0) {
		...
		if (!empty($_REQUEST['keywords'])) {
			...
			foreach ($arr as $key => $val) {
				...
				$keywords .= '(g.goods_name LIKE \'%' . $val . '%\' OR goods_sn LIKE \'%' . $val . '%\' OR keywords LIKE \'%' . $val . '%\' ' . $sc_dsad . ')';
			...
			foreach ($arr as $key => $val) {
				...
				$val = mysql_like_quote(trim($val));
				$act_name .= '(pa.goods_name LIKE \'%' . $val . '%\' OR pa.act_name LIKE \'%' . $val . '%\')';
			}
		...
		$sql = 'SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' AS g ' . $leftJoin . 'WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . $attr_in . ' ' . 'AND (( 1 ' . $categories . $keywords . $where_brand . $where_price_min . $where_price_max . $intro . $outstock . ' ) ' . $tag_where . ' ) ' . $act_name . ' ' . $cou_goods_where . ' ';
		$count = $db->getOne($sql);
		...
		$sql = 'SELECT pa.*, g.goods_id, g.is_shipping, g.user_id, g.goods_name, g.shop_price,g.market_price, g.is_new, g.comments_number, g.sales_volume, g.is_best, g.is_hot,g.store_new, g.store_best, g.store_hot, ' . $shop_price . ' IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' . 'IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\'), g.shop_price * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, ' . ' IF(g.model_price < 1, g.goods_number, IF(g.model_price < 2, wg.region_number, wag.region_number)) AS goods_number, ' . 'g.promote_start_date, g.promote_end_date, g.is_promote, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type ' . 'FROM ' . $ecs->table('goods') . ' AS g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . 'WHERE (g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . $attr_in . ' ' . $cou_goods_where . ' ' . 'AND (( 1 ' . $categories . $keywords . $where_brand . $where_price_min . $where_price_max . $intro . $outstock . ' ) ' . $tag_where . ' ) ) ' . $act_name . $where_price_min . $where_price_max . ' GROUP BY g.goods_id ORDER BY g.' . $sort . ' ' . $order;
		if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'load_more_goods')) {
		...
		while ($row = $db->FetchRow($res)) {
			...
			if ($row['act_id']) {
				...
				$arr[$idx]['pshop_url'] = build_uri('merchants_index', array('merchant_id' => $row['ru_id']), $row['shop_name']);
