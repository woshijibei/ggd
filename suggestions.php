...
if (empty($keyword)) {
	...
	foreach ($cate_res as $key => $row) {
		if (0 < $row['parent_id']) {
			...
			$cat_html .= '<li onmouseover="_over(this);" onmouseout="_out(this);">'. $_LANG['cat_search']  . '<a class=\'cate_user\' href=' . $url . ' style=\'color:#ec5151;\'>' . $parent_res['cat_name'] . '>' . $row['cat_name'] . '</a>' . '</li>';
		}
	}
	...
	while ($row = $db->FetchRow($result)) {
		...
		$html .= '<li onmouseover="_over(this);" title="' . $row['keyword'] . '" onmouseout="_out(this);" onClick="javascript:fill(\'' . $row['keyword'] . '\');"><div class="left-span">&nbsp;' . $keyword_style . '</div><div class="suggest_span">' . $count . ' Items</div></li>';
		$res_num++;
	...
	if ($res_num < 10) {
		...
		for ($i = 0; $i < $len; $i++) {
			...
			$html .= '<li onmouseover="_over(this);" onmouseout="_out(this);" title="' . $keyword_new_name . '" onClick="javascript:fill(\'' . $keyword_new_name . '\');"><div class="left-span">&nbsp;' . $keyword_style . '</div>&nbsp;<b>' . '</b>' . '<div class="suggest_span">' . $count . ' Items</div></li>';
		}
	}
