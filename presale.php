...
{
	$now = gmtime();
	$sql = 'SELECT a.*, g.goods_thumb, g.goods_img, g.goods_name, g.shop_price, g.market_price, g.sales_volume, s.* FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS a ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' AS s ON a.user_id = s.ru_id ' . 'WHERE a.cat_id = \'' . $cat_id . '\' AND g.is_on_sale = 0 AND a.review_status = 3 AND a.start_time <= \'' . $now . '\' AND a.end_time >= \'' . $now . '\' AND g.goods_id <> \'' . $goods_id . '\' ORDER BY g.click_count DESC LIMIT 5 ';
	$res = $GLOBALS['db']->getAll($sql);
...
if ($_REQUEST['act'] == 'index') {
	...
	if (!empty($_REQUEST['act']) && ($_REQUEST['act'] == 'guess_goods')) {
		...
		if (!$smarty->is_cached('presale_goods.dwt', $cache_id)) {
			...
			$smarty->assign('helps', get_shop_help());
			$smarty->assign('look_top', get_top_presale_goods($goods_id, $presale['pa_catid']));
		...
		$cart = array('user_id' => $_SESSION['user_id'], 'session_id' => $sess, 'goods_id' => $presale['goods_id'], 'product_id' => $product_info['product_id'], 'goods_sn' => addslashes($goods['goods_sn']), 'goods_name' => addslashes($goods['goods_name']), 'market_price' => $goods['market_price'], 'goods_price' => get_final_price($presale['goods_id']), 'goods_number' => $number, 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $specs, 'ru_id' => $goods['user_id'], 'warehouse_id' => $region_id, 'area_id' => $area_id, 'is_real' => $goods['is_real'], 'extension_code' => 'presale', 'parent_id' => 0, 'rec_type' => CART_PRESALE_GOODS, 'is_gift' => 0);
