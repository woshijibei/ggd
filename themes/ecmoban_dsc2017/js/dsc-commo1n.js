...
$(function(){
	...
	$(document).on("click","*[ectype='coll_brand']",function(){
		...
		if(user_id > 0){
			...
			if($(this).find("i").hasClass("icon-zan-alts")){
				...
				$(this).find("*[ectype='follow_span']").html("Following");
					...
					onOk:function(){
						...
						if(store_id > 0){
							if(store_mobile == ""){
								...
								return false;
							}else if(!Utils.isTel(store_mobile)){
	...
	$("*[ectype='bankList'] li").on("click",function(){
		...
		if(parent.find(".selected").length>0){
			...
			$("#alipay_bank").find("input").show().css({"background-color":"#e62e04"});
		}
	});
...
function addUpdate_Consignee(frm){
	...
	}else if(!Utils.isTel(csg.mobile)){
		pbDialog(json_languages.msg_phone_not,'',0);
		return false;
...
function regionSelect(ru_id,goods_id){
	...
	.off("click",catyHst).on('click',catyHst,function(){
		var spec_arr = '';
		var formBuy = document.forms['ECS_FO$BUY'];
		if (formBuy)
		{
			spec_arr = getSelectedAttributes(formBuy);
		}
		...
		if(data.error == 1){
			...
			if(data.type == 1){
				if(type == 'goods'){
					...
					obj.removeClass("selected");
					obj.html("<span>Follow</span><i class='iconfont icon-zan-alt'></i>");
				}else{
					obj.removeClass("selected");
					obj.html("<i class='iconfont icon-zan-alt'></i>Follow");
				}
				...
				if(type == 'goods'){
					$("[ectype='collect_store']").each(function(){
						...
						obj.html("<i class='iconfont icon-zan-alts'></i>Following");
					})
				}else if(type == 'store'){
					obj.addClass("selected");
					obj.html("<span>Following</span><i class='iconfont icon-zan-alts'></i>");
				}else{
					obj.addClass("selected");
					obj.html("<i class='iconfont icon-zan-alts'></i>Following");
				}
...
function goodsCollectStorenResponse(res){
	...
	if(res.error > 0){
		...
		if($(".gz-store").length > 0){
			$(".gz-store").html('<i class="iconfont icon-zan-alts"></i>Following');
			$(".gz-store").addClass('selected');
		}
		
		if($(".gz-store-top").length > 0){
			$(".gz-store-top").html('<span>Following</span><i class="iconfont icon-zan-alts"></i>');
			$(".gz-store-top").addClass('selected');
		}
	}else{
		
		if($(".gz-store").length > 0){
			$(".gz-store").html('<i class="iconfont icon-zan-alt"></i>Follow');
			$(".gz-store").removeClass('selected');
		}
		
		if($(".gz-store-top").length > 0){
			$(".gz-store-top").html('<span>Follow</span><i class="iconfont icon-zan-alt"></i>');
