...
class Inflector
{
	...
	static private $singular = array(
		...
		'uninflected' => array('.*[nrlm]ese', '.*deer', '.*fish', '.*measles', '.*ois', '.*pox', '.*sheep', '.*ss'),
		'irregular'   => array('criteria' => 'criterion', 'curves' => 'curve', 'emphases' => 'emphasis', 'foes' => 'foe', 'hoaxes' => 'hoax', 'media' => 'medium', 'neuroses' => 'neurosis', 'waves' => 'wave', 'oases' => 'oasis')
		);
	/**
     ...
     * @var array
     */
	static private $initialState = array();

	static public function tableize($word)
	{
	...
	static public function rules($type, $rules, $reset = false)
	{
		foreach ($rules as $rule => $pattern) {
			...
			unset($rules[$rule]);
			unset(self::$$type['cache' . ucfirst($rule)]);
