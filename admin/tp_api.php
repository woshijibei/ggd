...
function get_order_print_setting($ru_id)
{
	...
	$filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'ops.id' : trim($_REQUEST['sort_by']);
...
if (empty($_REQUEST['act'])) {
	...
	foreach ($order_ids as $order_id) {
		$result = get_kdniao_print_content($order_id);
	...
	if (($_REQUEST['act'] == 'order_print_setting_add') || ($_REQUEST['act'] == 'order_print_setting_edit')) {
		...
		if (($_REQUEST['act'] == 'order_print_setting_insert') || ($_REQUEST['act'] == 'order_print_setting_update')) {
			...
			if (empty($print_specification)) {
				$print_specification = get_table_date('order_print_setting', 'ru_id=\'' . $adminru['ru_id'] . '\'', array('specification'), 2);
			}

			$print_size_info = get_table_date('order_print_size', 'specification=\'' . $print_specification . '\'', array('*'));
			$print_size_list = get_table_date('order_print_setting', 'ru_id=\'' . $adminru['ru_id'] . '\'', array('*'), 1);
			...
			if (empty($print_size_list)) {
				...
				sys_msg($_LANG['no_print_setting'], 0, $link);
			}
			...
			foreach ($order_ids as $order_id) {
				$order_info = get_print_order_info($order_id);
