...
if ($_REQUEST['act'] == 'order_query') {
	...
	if ($order_finish) {
		...
		if (($GLOBALS['_CFG']['sms_order_shipped'] == '1') && ($order['mobile'] != '')) {
			...
			$smsParams = array('shop_name' => $shop_name, 'user_name' => $user_info['user_name'], 'consignee' => $order['consignee'], 'order_sn' => $order['order_sn'], 'mobile_phone' => $order['mobile'], 'invoice_no' => $invoice_no);
	...
	if (file_exists($file) && (0 < $order['user_id'])) {
		...
		$url = $GLOBALS['ecs']->url() . 'mobile/?r=wechat/api&user_id=' . $order['user_id'] . '&code=' . urlencode($code) . '&pushData=' . $data . '&url=' . $order_url;
		curlGet($url);
	}
