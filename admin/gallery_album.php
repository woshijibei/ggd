...
if ($_REQUEST['act'] == 'list') {
	...
	if (0 < $parent_id) {
		...
		$smarty->assign('action_link1', array('text' => '返回上级', 'href' => 'gallery_album.php?act=list&parent_id=' . $parent_album_id));
	}
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if ($_REQUEST['act'] == 'add') {
			...
			$cat_select = gallery_cat_list(0, $cat_info['parent_id'], false, 0, true, $album_info['ru_id']);
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if (!empty($checkboxes)) {
				...
				if ($db->query($sql) == true) {
					...
					if (!empty($unremove_arr)) {
						$sql = 'SELECT album_mame FROM' . $ecs->table('gallery_album') . 'WHERE album_id' . db_create_in($checkboxes);
			...
			$sql = 'SELECT ru_id FROM' . $ecs->table('gallery_album') . 'WHERE album_id = \'' . $album_id . '\' LIMIT 1';
			$seller_id = $db->getOne($sql);
			$cat_select = gallery_cat_list(0, 0, false, 0, true, '', 1, $seller_id);
