...
function lib_goods_list($real_goods = 1, $conditions = '', $review_status = 0, $real_division = 0)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods_lib') . ' AS g ' . $leftjoin . ' WHERE ' . $where . ' ' . $groupby;
		if (($filter['cat_type'] != 'seller') && $filter['cat_id']) {
			$filter['record_count'] = count($GLOBALS['db']->getAll($sql));
		}
		else {
		...
		$filter = page_and_size($filter);
		$sql = ' SELECT g.goods_id, g.lib_cat_id, g.goods_thumb, g.goods_name, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.sort_order, g.is_real, g.bar_code, g.is_on_sale ' . ' FROM ' . $GLOBALS['ecs']->table('goods_lib') . ' AS g ' . $leftjoin . ' WHERE  ' . $where . ' GROUP BY g.goods_id' . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
		$filter['keyword'] = stripslashes($filter['keyword']);
		set_filter($filter, $sql, $param_str);
	}
	else {
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if (strpos($row[$i]['goods_thumb'], $http) === false) {
			$row[$i]['goods_thumb'] = $GLOBALS['ecs']->url() . $row[$i]['goods_thumb'];
		}
