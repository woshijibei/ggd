...
function checkOrderResponse(result)
{
  ...
  try
  {
	  var total = parseInt(result.new_orders)+parseInt(result.new_paid)+parseInt(result.user_account)+parseInt(result.shop_account)+parseInt(result.shopinfo_account)+parseInt(result.no_paid)+parseInt(result.no_change)+parseInt(result.wait_cash)+parseInt(result.advance_date)+parseInt(result.goods_report)+parseInt(result.complaint);
	 $("#total").html(total);
	 $("#new_orders").html(result.new_orders);
	 $("#new_paid").html(result.new_paid);
	 $("#shop_account").html(result.shop_account);
	 $("#shopinfo_account").html(result.shopinfo_account);
	 $("#user_account").html(result.user_account);
	 $("#no_paid").html(result.no_paid);
	 $("#no_change").html(result.no_change);
	 $("#wait_cash").html(result.wait_cash);
	 $("#advance_date").html(result.advance_date);
         $("#goods_report").html(result.goods_report);
         $("#complaint").html(result.complaint);
  }
  catch (e) { }
}
...
//设置属性表格
function wholesale_set_attribute_table(goodsId)
{
	var attr_id_arr = [];
	var attr_value_arr = [];
