...
function newPrice(id) {
    $.get('index.php?c=snatch&a=new_price_list&id=' + id, '',
            function(data) {
...
function sendHashMail() {
    $.get('index.php?c=user&a=send_hash_mail', '', function(data) {
...
function orderQuery() {
    ...
    $.get('index.php?c=user&a=order_query&order_sn=s' + order_sn, '',
            function(data) {
...
function pmwin(action, param) {
    ...
    if (action == 'open') {
        ...
        $$('pmlayer').style.top = ((clientHeight - pmheight) / 2 + scrollTop)
                + 'px';
        if (!param) {
            pmframe.location = 'index.php?c=pm';
            //pmframe.location = 'pm.php';
        } else {
            pmframe.location = 'index.php?c=pm&' + param;
 ...
 * 添加礼包到购物车
 */
function addPackageToCart(packageId, areaId, regionId) {
    var package_info = new Object();
    var number = 1;

    package_info.package_id = packageId;
    package_info.number = number;
    package_info.area_id = areaId;
    package_info.warehouse_id = regionId;
    ...
    if (result.error > 0) {
        if (result.error == 2) {
            layer.open({
                ...
                shadeClose: false,
                yes: function () {
                    window.location.href = ROOT_URL + 'index.php?m=user&a=add_booking&id=' + result.package_id;
                },
                no: function () {
                }
            });
...
function addToCart_quick(goodsId, parentId) {
    ...
    //Ajax.call('index.php?c=flow&a=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse_quick, 'POST', 'JSON');
...
function addToCartResponse_quick(result) {
    ...
    if (result.error > 0) {
        ...
        // 更新购物车数量
        d_messages("Item has been added to the shopping cartz！", 2);
        $(".cart-num").html(result.goods_number);
 ...
 * 添加商品到购物车-购物车
 */
function in_addToCart(goodsId, parentId)
{
  var goods        = new Object();
  var spec_arr     = new Array();
  var fittings_arr = new Array();
  ...
  // 检查是否有商品规格
  if ($(".goods-size").hasClass() || $(".goods-color").hasClass())
  {
	quick = 1;
  }
  ...
  goods.number   = number;
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
  $.ajax({ url: ROOT_URL + "index.php?m=cart&a=add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
          if(data.error==0){
        	  d_messages('已加入购物车', 2);
          }else{
...
var quick = 1;
  ...
  $("label.ts-1"+id).each(function (){
    ...
    goods.goods_id = id;
    goods.spec = str.substr(0,str.length-1).split(',');
    goods.number = $(".attr-number"+id).val();
    goods.warehouse_id = $("input[name=warehouse_id]").val();
    goods.area_id = $("input[name=area_id]").val();
    goods.quick = quick;
    $.ajax({ url: ROOT_URL + "index.php?m=cart&a=add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
        if(data.error==0){
            d_messages('已加入购物车', 2);
        ...
        window.location=ROOT_URL + 'index.php?m=cart';//刷新当前页面.
    }});
}
var strr = '';
function addtocart_list(id){
    var goods = new Object();
    var quick = 1;

    $("label.ts-1"+id).each(function (){
        if($(this).hasClass("active")){
            strr += $(this).attr("attr-id")+',';
        }
    })
    goods.goods_id = id;
    goods.spec = strr.substr(0,strr.length-1).split(',');
    goods.number = $("input[name=number"+id+"]").val();
    goods.warehouse_id = $("input[name=warehouse_id]").val();
    goods.area_id = $("input[name=area_id]").val();
    goods.quick = quick;
    $.ajax({ url: ROOT_URL + "index.php?m=cart&a=add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
