...
function T($template='',$layer=''){
    ...
    if($auto && isset($auto[$extend])){ // 扩展资源
        ...
        // 指定全局视图目录
        $baseUrl    =   strtolower(TMPL_PATH.parse_name($module));
    }else{
    ...
    // 开发模块模板文件
    $tpl = PLAYGROUND_PATH . $module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    $tpl = PLAYGROUND_PATH . $module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    // 应用模板文件
    $tpl = APP_PATH . $module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    $tpl = APP_PATH . $module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    // 兼容模板文件
...
function controller($name,$path=''){
    ...
    if(!C('APP_USE_NAMESPACE')){
        ...
        $class  =   ( $path ? basename(ADDON_PATH).'\\'.$path : MODULE_NAME ).'\\'.$layer . 's';
        $array  =   explode('/',$name);
        foreach($array as $name){
            $class  .=   '\\'.parse_name($name, 1);
        }
            ...
            if(!empty($path)) {
                ...
                if(C('MULTI_MODULE')) {
                    if(MODULE_NAME != C('DEFAULT_MODULE') || !C('MODULE_ALLOW_LIST')){
                        $var[$varModule] = parse_name(MODULE_NAME);
                    }
                }
            }
