...
class Admin extends \App\Http\Base\Controllers\Backend
{
	...
	public function actionDrpSetQrcode()
	{
		if (IS_POST) {
			...
			if (!empty($pic_path)) {
				...
				//print_r($img_info);die("aaa");
				//if (($img_info[0] < 640) || ($img_info[1] < 1136)) {
					//exit(json_encode(array('error' => 1, 'msg' => '图片宽高不符合规定')));
				//}
			}
			...
			if ($_FILES['pic']['name']) {
				...
				$result = $this->upload('data/attached/qrcode/themes', true, 1,false,false);
	...
	public function actionDrpUserCreditEdit()
	{
		...
		if (IS_AJAX) {
			...
			$result = array('error' => 1, 'msg' => 'Select');
			exit(json_encode($result));
		}
