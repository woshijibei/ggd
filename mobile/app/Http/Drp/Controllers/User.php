...
class User extends \App\Http\Base\Controllers\Frontend
{
	...
	public function actionCategory()
	{
		...
		$this->assign('category', $category);
		$this->assign('page_title', $this->custom . '{$lang.category}');
		$this->display();
	}
	...
	public function actionDrpchildcategory()
	{
		if (IS_AJAX) {
			if (empty($this->cat_id)) {
				exit(json_encode(array('code' => 1, 'message' => 'Select分类')));
			}
	...
	public function actionUserCard()
	{
		...
		if ($generate) {
			...
			if ($qrcodeInfo) {
				...
				//$bgImg="/www/wwwroot/www.xibugw.com/data/attached/qrcode/themes/59a0e0f708e57.png"; 
                //https://xbgw.oss-cn-shanghai.aliyuncs.com/data/attached/qrcode/themes/59a0e0f708e57.png?v=1503686962
				
				if($bgImg!=$file . 'drp_bg.png'){                   
                    $arr=explode('?v=',$bgImg);
		...
		$image_name = '/data/attached/qrcode/' . basename($outImg1);                  
