...
function get_coupons_type_info($cou_type = '1,2,3,4', $ru_id = '')
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$row['cou_is_use'] = $row['cou_is_use'] == 0 ? 'Available' : '<span style=color:red;>Used</span>';
		$row['cou_is_time'] = gmtime() < local_strtotime($row['cou_end_time']) ? 'Available' : '<span style=color:red;>Overdue</span>';
		$arr[] = $row;
	}
...
function cancel_return($ret_id, $user_id = 0)
{
	...
	if ($GLOBALS['db']->query($sql)) {
		...
		$GLOBALS['db']->query($sql);
		return_action($ret_id, 'Cancel', '', '', '买家', '');
		return true;
	}
	else {
		exit($GLOBALS['db']->errorMsg());
