...
class Index extends \App\Http\Base\Controllers\Frontend
{
	...
	public function __construct()
	{
		...
		$cart_value = dao('cart')->where($this->sess_id . ' AND is_checked = 1')->getField('rec_id', true);
		$_SESSION['cart_value'] = empty($cart_value) ? '' : implode(',', $cart_value);
	}
	...
	public function actionIndex()
	{
		...
		if (isset($payment_list)) {
			foreach ($payment_list as $key => $payment) {
				...
				if (!is_dir(APP_WECHAT_PATH) && ($payment['pay_code'] == 'wxpay')) {
					unset($payment_list[$key]);
				}
			}
		}
	...
	public function actionDone()
	{
		...
		if (isset($is_real_good)) {
			if ((empty($order['shipping_id']) && empty($point_id) && empty($store_id)) || empty($order['pay_id'])) {
				show_message('Please select the shopping method or payment option.');
			}
		}
	...
	{
		if (IS_POST) {
			$consignee = array('address_id' => I('address_id'), 'consignee' => I('consignee'), 'country' => I('country_region_id'), 'province' => I('province_region_id'), 'city' => I('city_region_id'), 'district' => I('district_region_id'), 'street' => I('town_region_id'), 'email' => I('email'), 'address' => I('address'), 'zipcode' => I('zipcode'), 'tel' => I('tel'), 'mobile' => I('mobile'), 'sign_building' => I('sign_building'), 'best_time' => I('best_time'), 'user_id' => $_SESSION['user_id']);
	...
	{
		if (IS_POST) {
			$consignee = array('address_id' => I('address_id'), 'consignee' => I('consignee'), 'country' => I('country_region_id'), 'province' => I('province_region_id'), 'city' => I('city_region_id'), 'district' => I('district_region_id'), 'email' => I('email'), 'address' => I('address'), 'zipcode' => I('zipcode'), 'tel' => I('tel'), 'mobile' => I('mobile'), 'sign_building' => I('sign_building'), 'best_time' => I('best_time'), 'user_id' => $_SESSION['user_id']);
