...
class Extend extends \App\Http\Base\Controllers\Backend
{
	...
	public function actionEdit()
	{
				...
				if (empty($handler) && !empty($rs['enable'])) {
					...
					if (empty($cfg_value['media_id'])) {
						...
						if ($media_id) {
							...
							$sql_file = MODULE_PATH . 'plugins/' . $this->plugin_name . '/install.sql';

							if (file_exists($sql_file)) {
								$sql = file_get_contents($sql_file);
								$sql = str_replace(array('ecs_wechat_media', '(0', 'http://', 'views/images'), array('{pre}wechat_media', '(' . $this->wechat_id, __HOST__ . url('wechat/index/plugin_show', array('name' => $this->plugin_name)), 'public/assets/wechat/' . $this->plugin_name . '/images'), $sql);
		...
		if (!empty($handler)) {
			...
			if (empty($info)) {
				$this->message('Select要编辑的功能扩展', null, 2);
			}

			$info['config'] = unserialize($info['config']);
		}
  
		$plugin = '\\App\\Http\\Wechat\\Plugins\\' . $this->plugin_name . '\\' . ucfirst($this->plugin_name); 

		if (class_exists($plugin)) { 
	        $plugin=str_replace('App','app',$plugin);  
	...
	public function actionUninstall()
	{
		...
		if (empty($keywords)) {
			$this->message('Select要卸载的功能扩展', null, 2);
		}
	...
	public function actionWinnerIssue()
	{
		...
		if (empty($id)) {
			$this->message('Select中奖记录', null, 2);
		}
