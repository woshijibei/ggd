...
class Sellerextend extends \App\Http\Base\Controllers\BackendSeller
{
	...
	public function actionEdit()
	{
		...
		if (!empty($handler)) {
			...
			if (empty($info)) {
				$this->message('Select要编辑的功能扩展', null, 2, true);
			}
	...
	public function actionUninstall()
	{
		...
		if (empty($keywords)) {
			$this->message('Select要卸载的功能扩展', null, 2, true);
		}
	...
	public function actionWinnerList()
	{
		...
		if (empty($ks)) {
			$this->message('Select插件', null, 2, true);
		}
	...
	public function actionWinnerIssue()
	{
		...
		if (empty($id)) {
			$this->message('Select中奖记录', null, 2, true);
		}
