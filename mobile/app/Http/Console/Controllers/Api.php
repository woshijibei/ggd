...
class Api extends \App\Http\Admin\Controllers\Editor
{
	...
	public function actionProduct()
	{
		if (IS_POST) {
			...
			if ($number == 0) {
				$wherenum = ' LIMIT ' . $currentPage . ' , ' . $pageSize . ' ';
			}
			else {
				$wherenum = ' LIMIT ' . $number . ' ';
			}
			...
			$this->response(array('error' => 0, 'product' => $product, 'total' => $pageSize, 'page' => $count));
		}
	}
	...
	public function actionUrl()
	{
		if (IS_POST) {
			...
			if ($type == 'function') {
				...
				$cart = array('cat_id' => '2', 'cat_name' => 'Shopping Cart', 'parent_id' => 0, 'url' => url('cart/index/index'));
				...
				$exchange = array('cat_id' => 2, 'cat_name' => 'Points', 'parent_id' => 0, 'url' => url('exchange/index/index'));
				...
				$activity = array('cat_id' => 5, 'cat_name' => 'Deals', 'parent_id' => 0, 'url' => url('activity/index/index'));
			...
			if (empty($new)) {
				...
				if (!empty($index)) {
					...
					if (empty($new)) {
						...
						$keep = array('ru_id' => 0, 'type' => 'index', 'page_id' => 0, 'title' => 'Home', 'data' => $index, 'default' => 1, 'review_status' => 3, 'is_show' => 1);
		...
		if (IS_POST) {
			...
			if (empty($view)) {
				...
				if (empty($new)) {
					...
					$view = array('ru_id' => 0, 'type' => 'index', 'title' => 'Home', 'data' => $data, 'default' => 1);
	...
	public function actionOldSave()
	{
		...
		if ($authority == 1) {
			if (IS_POST) {
				$time = gmtime();
				$keep = array('ru_id' => 0, 'title' => 'Home', 'type' => 'index', 'data' => !empty($_POST['data']) ? $_POST['data'] : '', 'update_at' => $time, 'default' => 1);
	...
	public function actionRestore()
	{
		...
		if ($authority == 1) {
			$data = str_replace('<?php exit("no access");', '', file_get_contents(ROOT_PATH . 'storage/app/diy/default.php'));
			$keep = array('type' => 'index', 'title' => 'Home', 'data' => $data);
	...
	public function actionNavigation()
	{
		if (IS_POST) {
			...
			F('navigation', $navigation, $path = DATA_PATH);
		}
