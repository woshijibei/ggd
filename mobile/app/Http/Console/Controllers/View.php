...
class View extends \App\Http\Base\Controllers\Frontend
{
		...
		if (IS_POST) {
			...
			if (empty($article_msg)) {
				...
				if ($cid == 0) {
					...
					$where = array('cat_id' => $cid, 'is_open' => 1);
				}
			...
			if ($ru_id) {
				...
				if ($index) {
					...
					if (!empty($index)) {
						...
						$keep = array('ru_id' => 0, 'type' => 'index', 'title' => 'Home', 'data' => $data, 'default' => 1);
	...
	public function actionSeckill()
	{
		...
		if (!empty($sec)) {
			foreach ($sec as $key => $val) {
				$sql = 'SELECT stb.* ' . ' FROM ' . $GLOBALS['ecs']->table('seckill_time_bucket') . ' AS stb ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg ' . 'ON stb.id = sg.tb_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ' . 'ON sg.sec_id = s.sec_id ' . ' WHERE s.is_putaway = 1 AND s.begin_time < ' . $now . ' AND s.acti_time > ' . $now . ' AND stb.id = ' . $val['id'];
		...
		$sql = 'SELECT sg.id, sg.tb_id, sg.goods_id, sg.sec_price, sg.sec_num ' . ' FROM ' . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ' . 'ON sg.sec_id = s.sec_id ' . ' WHERE s.is_putaway = 1 AND sg.tb_id = ' . $arr['id'];
	...
	public function actionStoreDown()
	{
		if (IS_POST) {
			$ru_id = input('ruid');
			$time = gmtime();
			$sql = 'SELECT ms.shop_id, ms.user_id, ms.rz_shopName,ss.kf_qq, ss.kf_ww, ss.meiqia ' . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS ms ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' AS ss ' . ' ON ms.user_id = ss.ru_id  ' . ' WHERE ms.user_id = ' . $ru_id . '  ';
			...
			foreach ($shop as $key => $value) {
				...
				if ($value['kf_qq']) {
					...
					$store[$key]['kf'] = 'https://static.meiqia.com/dist/standalone.html?eid=' . $value['meiqia'];
				}
			}
