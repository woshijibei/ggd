...
function get_collection_store_list($user_id, $record_count, $limit = '')
{
	...
	foreach ($res as $key => $row) {
		...
		if (4 <= $store_list[$key]['commentrank']) {
			$store_list[$key]['rankgoodReview'] = 'A';
		}
		else if (3 < $store_list[$key]['commentrank']) {
			$store_list[$key]['rankgoodReview'] = 'B';
		}
		else {
			$store_list[$key]['rankgoodReview'] = 'C';
		}

		if (4 <= $store_list[$key]['commentServer']) {
			$store_list[$key]['ServergoodReview'] = 'A';
		}
		else if (3 < $store_list[$key]['commentServer']) {
			$store_list[$key]['ServergoodReview'] = 'B';
		}
		else {
			$store_list[$key]['ServergoodReview'] = 'C';
		}

		if (4 <= $store_list[$key]['commentdelivery']) {
			$store_list[$key]['deliverygoodReview'] = 'A';
		}
		else if (3 < $store_list[$key]['commentdelivery']) {
			$store_list[$key]['deliverygoodReview'] = 'B';
		}
		else {
			$store_list[$key]['deliverygoodReview'] = 'C';
		}
