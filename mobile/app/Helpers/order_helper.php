...
function get_cart_goods($cart_value = '', $type = 0, $warehouse_id = 0, $area_id = 0, $favourable_list = array())
{
	if (!empty($_SESSION['user_id'])) {
		$sess_id = ' user_id = \'' . $_SESSION['user_id'] . '\' ';
	}
...
function compute_discount($type = 0, $newInfo = array(), $use_type = 0, $ru_id = 0)
{
	...
	if (($type == 0) || ($type == 3)) {
		...
		$sql = 'SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, c.ru_id ' . 'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE c.goods_id = g.goods_id ' . 'AND ' . $c_sess . 'AND c.parent_id = 0 ' . 'AND c.is_gift = 0 ' . 'AND c.is_checked = 1 ' . 'AND rec_type = \'' . CART_GENERAL_GOODS . '\'' . $where;
		$goods_list = $GLOBALS['db']->getAll($sql);
	}
	else if ($type == 2) {
		$goods_list = array();
...
function get_virtual_goods_info($rec_id = 0)
{
	...
	if ($virtual_info) {
		$virtual_info['card_sn'] = _decrypt($virtual_info['card_sn']);
		$virtual_info['card_password'] = _decrypt($virtual_info['card_password']);
