...
class OrderRepository implements \App\Contracts\Repository\Order\OrderRepositoryInterface
{
	...
	public function orderNum($id, $status = NULL)
	{
		...
		if ($status === \App\Models\OrderInfo::STATUS_CREATED) {
		...
		if (!empty($status)) {
			...
			case \App\Models\OrderInfo::STATUS_PAID:
				$model->wherein('pay_status', array(\App\Models\OrderInfo::PS_PAYED));
				break;
	...
	public function orderConfirm($uid, $orderId)
	{
		...
		$order->shipping_status = \App\Models\OrderInfo::SS_RECEIVED;
	...
	{
		$array = array('order_status' => \App\Models\OrderInfo::OS_CONFIRMED, 'pay_status' => \App\Models\OrderInfo::PS_PAYED, 'pay_time' => gmtime(), 'money_paid' => \Illuminate\Support\Facades\DB::Raw('order_amount'), 'order_amount' => 0);
	...
	public function getOrderByUserId($id, $status = 0, $page = 0, $size = 10)
	{
		...
		$order = $model->select(array('order_id', 'order_sn', 'order_status', 'shipping_status', 'pay_status', 'goods_amount', 'order_amount', 'add_time', 'shipping_status', 'shipping_status', 'money_paid', 'shipping_fee'))->with(array('goods' => function($query) {
