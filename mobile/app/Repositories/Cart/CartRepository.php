...
class CartRepository implements \App\Contracts\Repository\Cart\CartRepositoryInterface
{
	...
	{
		$cart = \App\Models\Cart::where('user_id', $uid)->where('goods_id', $goodsId)->where('goods_attr_id', $goodsAttr)->first();
	...
	{
		return \App\Models\Cart::where('user_id', $id)->sum('goods_number');
	}
