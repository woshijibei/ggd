	...
	public function callback($callback_url, $code)
	{
		if (!empty($code)) {
			...
			if (!empty($userinfo) && !empty($userinfo['unionid'])) {
				...
				$data = array('openid' => $userinfo['unionid'], 'nickname' => $userinfo['nickname'], 'sex' => $userinfo['sex'], 'headimgurl' => $userinfo['headimgurl'], 'city' => $userinfo['city'], 'province' => $userinfo['province'], 'country' => $userinfo['country']);
