...
class alipay
{
	...
	{
		$payData = array('body' => $order['order_sn'], 'subject' => $order['order_sn'], 'order_no' => $order['order_sn'] . 'O' . $order['log_id'], 'timeout_express' => time() + (3600 * 24), 'amount' => $order['order_amount'], 'return_param' => $order['log_id'], 'client_ip' => isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1', 'goods_type' => 1, 'store_id' => '');
