...
function get_users_log_list($num = 10, $start = 0, $user_id = 0)
{
	...
	foreach ($row as $k => $v) {
		...
		if (0 < $v['admin_id']) {
			...
			$row['admin_name'] = '管理员：' . $GLOBALS['db']->getOne($sql);
		}
	}
...
if (in_array($action, $ui_arr)) {
	...
	$sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('shop_config') . ' WHERE code = \'is_illegal\'';
	$is_illegal = $db->getOne($sql);
	$smarty->assign('is_illegal', $is_illegal);
...
if ($action == 'register') {
	...
	$smarty->assign('article_id', $_CFG['register_article_id']);
	...
	if ($c) {
		...
		if (!$url) {
			show_message($c->get_error(), 'Home', $ecs->url(), 'error');
		}

		header('Location: ' . $url);
	}
	else {
		show_message('服务器尚未注册该插件！', 'Home', $ecs->url(), 'error');
	}
}
	...
	$json = new JSON();
	$dsc_token = INPUT_I('post.dsc_token', '');
	if (isset($_SERVER['HTTP_USER_AGENT']) && $_SERVER['HTTP_USER_AGENT']) {
		$dsc_token = $dsc_token . MD5($_SERVER['HTTP_USER_AGENT']);
	}

	if (isset($_SESSION['token_agent']) && ($dsc_token != $_SESSION['token_agent'])) {
		$result['error'] = 1;
		$result['message'] = $_LANG['illegal_operate'];
	}
